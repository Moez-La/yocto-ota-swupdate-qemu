...
void get_uptime(char *buf) {
    ...
    sprintf(buf, "%02d:%02d:%02d", (int)uptime/3600, ((int)uptime%3600)/60, (int)uptime%60);
}
...
void get_rx_tx(long *rx, long *tx) {
    FILE *fp = fopen("/proc/net/dev", "r"); char line[256];
    if (!fp) { *rx=0; *tx=0; return; }
    while (fgets(line, sizeof(line), fp))
        if (strstr(line, "eth0")) { sscanf(line+strcspn(line,":")+1, "%ld %*d %*d %*d %*d %*d %*d %*d %ld", rx, tx); break; }
    fclose(fp);
}
...
void get_cpu(char *buf) {
    ...
    if (!fp) { strcpy(buf, "N/A"); return; }
    fscanf(fp, "%f", &load); fclose(fp); sprintf(buf, "%.2f", load);
}
...
void get_mem(long *total, long *free) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!strncmp(line,"MemAvailable:",13)) sscanf(line+13,"%ld",free);
    } fclose(fp);
}
...
int main() {
    ...
    print_header();
    while (1) {
        get_uptime(uptime); get_ip(ip); get_rx_tx(&rx,&tx);
        get_cpu(cpu); get_mem(&mem_total,&mem_free);
        printf("[%s] IP           : %s\n", uptime, ip);
